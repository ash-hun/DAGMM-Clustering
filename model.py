...
class Cholesky(torch.autograd.Function):
	...
	def backward(ctx, grad_output):
		...
		inner = torch.tril(torch.mm(l.t(), grad_output)) * torch.tril(
			1.0 - l.new(l.size(1)).fill_(0.5).diag())
...
class DaGMM(nn.Module):
	...
	def __init__(self, n_gmm=2, latent_dim=3):
		...
		layers = []
		layers += [nn.Linear(31, 21)]
		layers += [nn.Tanh()]
		layers += [nn.Linear(21, 11)]
		layers += [nn.Tanh()]
		layers += [nn.Linear(11, 5)]
		layers += [nn.Tanh()]
		...
		layers = []
		layers += [nn.Linear(1, 5)]
		layers += [nn.Tanh()]
		layers += [nn.Linear(5, 11)]
		layers += [nn.Tanh()]
		layers += [nn.Linear(11, 21)]
		layers += [nn.Tanh()]
		layers += [nn.Linear(21, 31)]
